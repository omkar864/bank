...
const LoanApplicationForm = ({translateToHindi, loanSchemes}: LoanApplicationFormProps) => {
  ...
  useEffect(() => {
    const autoTranslate = async () => {
      ...
      // Optional fields with type guard
      const husbandWifeNameValue = form.watch('husbandWifeName');
      if (husbandWifeNameValue && !form.watch('husbandWifeNameHindi')) {
          const translatedText = await translateToHindi(husbandWifeNameValue);
          form.setValue('husbandWifeNameHindi', translatedText);
      }

      const alternateMobileValue = form.watch('alternateMobileNumber');
      if (alternateMobileValue && !form.watch('alternateMobileNumberHindi')) {
          const translatedText = await translateToHindi(alternateMobileValue);
