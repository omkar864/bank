...
const LoanApplicationForm = ({translateToHindi, loanSchemes}: LoanApplicationFormProps) => {
  ...
  useEffect(() => {
    ...
    const safeTranslate = async (fieldName: keyof LoanApplicationFormValues, hindiFieldName: keyof LoanApplicationFormValues) => {
      ...
      // Ensure value is a non-empty string before translating
      if (typeof valueToTranslate === 'string' && valueToTranslate && !form.watch(hindiFieldName)) {
        const translatedText = await translateToHindi(valueToTranslate);
        form.setValue(hindiFieldName, translatedText as any);
      }
    };
    
    const autoTranslate = async () => {
      // Create a list of fields to translate
      const fieldsToTranslate: [keyof LoanApplicationFormValues, keyof LoanApplicationFormValues][] = [
        ['customerName', 'customerNameHindi'],
        ['dateOfBirth', 'dateOfBirthHindi'],
        ['fatherName', 'fatherNameHindi'],
        ['motherName', 'motherNameHindi'],
        ['husbandWifeName', 'husbandWifeNameHindi'],
        ['mobileNumber', 'mobileNumberHindi'],
        ['alternateMobileNumber', 'alternateMobileNumberHindi'],
        ['residentialAddress', 'residentialAddressHindi'],
        ['permanentAddress', 'permanentAddressHindi'],
        ['companyShopName', 'companyShopNameHindi'],
        ['companyShopAddress', 'companyShopAddressHindi'],
        ['identityDocument', 'identityDocumentHindi'],
        ['documentNumber', 'documentNumberHindi'],
        ['addressProofDocument', 'addressProofDocumentHindi'],
        ['guarantorName', 'guarantorNameHindi'],
        ['guarantorDocumentName', 'guarantorDocumentNameHindi'],
        ['guarantorDocumentNumber', 'guarantorDocumentNumberHindi'],
        ['guarantorMobileNumber', 'guarantorMobileNumberHindi'],
        ['guarantorAddress', 'guarantorAddressHindi'],
        ['annualIncome', 'annualIncomeHindi'],
        ['monthlyIncome', 'monthlyIncomeHindi'],
        ['loanAmountRequired', 'loanAmountRequiredHindi'],
      ];

      // Sequentially translate each field
      for (const [engField, hindiField] of fieldsToTranslate) {
        await safeTranslate(engField, hindiField);
      }
    };
    ...
    form, 
    translateToHindi,
    // Add all watched fields as dependencies to re-run the effect when they change
    form.watch('customerName'), form.watch('dateOfBirth'), form.watch('fatherName'),
    form.watch('motherName'), form.watch('husbandWifeName'), form.watch('mobileNumber'),
    form.watch('alternateMobileNumber'), form.watch('residentialAddress'), form.watch('permanentAddress'),
    form.watch('companyShopName'), form.watch('companyShopAddress'), form.watch('identityDocument'),
    form.watch('documentNumber'), form.watch('addressProofDocument'), form.watch('guarantorName'),
    form.watch('guarantorDocumentName'), form.watch('guarantorDocumentNumber'), form.watch('guarantorMobileNumber'),
    form.watch('guarantorAddress'), form.watch('annualIncome'), form.watch('monthlyIncome'),
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                  <div className="space-y-0.5">
                    ...
                    <CardDescription>Enable auto fine for late payments.</CardDescription>
                  </div>
                  <FormControl>
                    <Switch
                      checked={field.value}
